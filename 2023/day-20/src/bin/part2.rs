...
fn part1(input: &str) -> usize {
  ...
  let mut printed: HashMap<&str, usize> = HashMap::new();
      ...
      match modules.get_mut(current) {
        ...
        Some(Module::Con(c)) => {
          ...
          if c.memory.values().all(|v| *v) {
            if !printed.contains_key(current) {
              println!("{current} {press_count}");
              printed.insert(current, press_count);
            }
            c.next
              .iter()
              .for_each(|name| queue.push_back((name, false, current)));
